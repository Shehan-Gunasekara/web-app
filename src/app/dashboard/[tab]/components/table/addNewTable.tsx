...
import { Flex, Switch, Input, Form, Button, theme, Spin, Tooltip } from "antd";
...
import { CREATE_TABLE, GENERATE_TABLE_QR_CODE } from "@/lib/mutations/table";
...
function AddNewTable({ closeModal, tableNumbers, formRef }: CloseModalProps) {
  const {
    token: {
      colorTextBase,
      colorBgBase,
  ...
  const [createTable] = useMutation<any>(CREATE_TABLE);

  useEffect(() => {
    setStatus(false);
    setQRCode(undefined);
    setTableNo(undefined);
    setSeatsNo(undefined);
  }, [closeModal]);

  //generating QR code
  const generateQRCode = async () => {
    form
      .validateFields(["tableNumber"])
      .then(async (_values) => {
        try {
          ...
          // setShowQrCode(false);
          setQRCode(data.generateTableQrCode);
        } catch (_error) {
          setIsQrGenerating(false);
        }
      })
        ...
        <div style={style.addTableUpperSection}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              >
                ...
                >
                  ...
                  >
                    {tableNo ? (tableNo < 10 ? `0${tableNo}` : tableNo) : "00"}
                  </span>
                </div>
              </div>
          ...
          >
            ...
            <div>
              ...
              <MdTableRestaurant
                fontSize="1.5rem"
                style={{ position: "absolute", top: "27%", right: "8%" }}
              />
            </div>
          ...
          >
            {QRCode ? (
              <Button
                ...
                size="large"
                style={{
                  backgroundColor: isQrGenerating
                    ? colorTextDisabled
                    : colorTextBase,
                  color: colorBgBase,
                  fontWeight: 600,
              ...
              >
                ...
                >
                  {isQrGenerating ? (
                    <>
                      Regenerate QR <Spin style={{ marginLeft: "0.5rem" }} />
                    </>
                  ) : (
                    "Regenerate QR"
                  )}
              ...
              >
                ...
                >
                  {isQrGenerating ? (
                    <>
                      Generate QR <Spin style={{ marginLeft: "0.5rem" }} />
                    </>
                  ) : (
                    "Generate QR"
                  )}
            ...
            <PDFDownloadLink
              document={<QRCodesPDF table_number={tableNo} qr_code={QRCode} />}
